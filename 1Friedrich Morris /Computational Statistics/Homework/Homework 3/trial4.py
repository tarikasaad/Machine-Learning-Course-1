# -*- coding: utf-8 -*-
"""
Created on Sun Oct 15 14:14:06 2023
...
# Data Preprocessing and Naive Bayes Model
...
# Model selection: Naive Bayes (Gaussian Naive Bayes)
...
cross_val_scores = cross_val_score(nb_classifier, X, y, cv=5, scoring='neg_log_loss')

# Print the cross-validation scores (log loss)
print("Cross-validation Log Loss with Naive Bayes:", -cross_val_scores.mean())
...
test_predictions = nb_classifier.predict_proba(X_test)[:, 1]  # Probability of outcome=1

# You can also save these predictions to a CSV file
submission_df = pd.DataFrame({'id': test_data['id'], 'outcome': test_predictions})
submission_df.to_csv('horse_survival_predictions.csv', index=False)
