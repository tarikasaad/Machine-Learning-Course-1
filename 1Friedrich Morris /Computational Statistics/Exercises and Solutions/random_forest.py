...
tree_regr = DecisionTreeRegressor(min_samples_split=2)
tree_regr.fit(X_train, y_train)
bagged_regression_tree = BaggingRegressor(n_estimators=100, base_estimator=DecisionTreeRegressor())
bagged_regression_tree.fit(X_train, y_train)

RF_regression = RandomForestRegressor() #new
RF_regression.fit(X_train, y_train) #new
...
plt.plot(X_seq, bagged_regression_tree.predict(X_seq), color="brown")

plt.plot(X_seq, RF_regression.predict(X_seq), color="violet") #new
...
print("MSE of bagged decision trees (brown):", mse(bagged_regression_tree, X_validate, y_validate))
print("MSE of random forest (violet):", mse(RF_regression, X_validate, y_validate)) #new